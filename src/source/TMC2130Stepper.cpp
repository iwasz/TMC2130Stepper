...
TMC2130Stepper::TMC2130Stepper (device const *spi, spi_config const *spiCfg) : spi{spi}, spiCfg{spiCfg}
{
#ifdef TMC2130DEBUG
...
int TMC2130Stepper::writeCmd (uint8_t cmd, uint32_t value)
{
        value = sys_cpu_to_be32 (value);
        const struct spi_buf txBufs[] = {{.buf = &cmd, .len = 1}, {.buf = &value, .len = sizeof (value)}};
        const struct spi_buf_set txBufSet = {.buffers = txBufs, .count = 2};

        const struct spi_buf rxBuf = {.buf = &lastStatusResponse, .len = 1};
        const struct spi_buf_set rxBufSet = {.buffers = &rxBuf, .count = 1};

        return spi_transceive (spi, spiCfg, &txBufSet, &rxBufSet);
}

/****************************************************************************/

std::pair<uint32_t, int> TMC2130Stepper::readCmd (uint8_t cmd)
{
        const struct spi_buf txBuf = {.buf = &cmd, .len = 1};
        const struct spi_buf_set txBufSet = {.buffers = &txBuf, .count = 1};

        uint32_t ret{};
        const struct spi_buf rxBufs[] = {{.buf = &lastStatusResponse, .len = 1}, {.buf = &ret, .len = sizeof (ret)}};
        const struct spi_buf_set rxBufSet = {.buffers = rxBufs, .count = 2};
        int status = spi_transceive (spi, spiCfg, &txBufSet, &rxBufSet);

        return {ret, status};
}

// int TMC2130Stepper::send2130 (uint8_t addressByte, uint32_t *config)
// {
//         // if (uses_sw_spi) {
...
//         int errno = spi_read (spi, spiCfg, bufs);
...
uint16_t TMC2130Stepper::rms_current () { return (float)(irun () + 1) / 32.0 * (vsense () ? 0.180 : 0.325) / (Rsense + 0.02) / 1.41421 * 1000; }
